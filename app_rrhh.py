...
st.write("Sube los archivos de liquidaciones de dos meses consecutivos para detectar variaciones mayores al 5%, ítems nuevos o faltantes.")

# 2. Carga de Archivos
col1, col2 = st.columns(2)
with col1:
    archivo_mes_anterior = st.file_uploader("Sube el archivo del Mes Anterior (CSV)", type=['csv'])
with col2:
    archivo_mes_actual = st.file_uploader("Sube el archivo del Mes Actual (CSV)", type=['csv'])

# 3. Lógica de Procesamiento
if archivo_mes_anterior is not None and archivo_mes_actual is not None:
    try:
        df_anterior = pd.read_csv(archivo_mes_anterior)
        df_actual = pd.read_csv(archivo_mes_actual)
        ...
        st.success("Cruce de datos realizado con éxito.")
